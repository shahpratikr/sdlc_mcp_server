...
class WorkflowState:
    ...
    def __init__(self) -> None:
        ...
        self._test_passed: dict[str, bool] = (
            {}
        )  # PRD 3.5: last test run result per story
        self._test_override: set[str] = (
            set()
        )  # PRD 3.5: developer override of a failed run
    ...
    def record_test_result(self, issue_key: str, *, passed: bool) -> None:
        """Record a story's test run outcome, resetting any prior override. PRD 3.5."""
        self._test_passed[issue_key] = passed
        self._test_override.discard(issue_key)

    def mark_test_override(self, issue_key: str) -> None:
        """Record the developer's explicit override of a failed test run. PRD 3.5."""
        self._test_override.add(issue_key)

    def require_tests_passed(self, issue_key: str) -> None:
        """Block PR creation for a story on a failed, non-overridden run. PRD 3.5."""
        if (
